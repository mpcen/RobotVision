...
char **argv;
{
        int     i,j,p,q,s,t,mr, HIGH, LOW,centx,centy, quit,sum;
        int uno = 1;
        double  xmaskval, ymaskval,sumx,sumy,sig,maxival, percent;
        FILE    *fo1, *fo2,*fp1, *fopen(), *fo3, *fo4;
        char    *foobar;
...
//        ZEROTOL = atof(foobar);
        ...
        { for (j=0;j<256;j++)
            {
            fprintf(fo3,"%c",(char)((int)(mag[i][j])));
        ...
        { for (j=0;j<256;j++)
            {
            fprintf(fo4,"%c",(char)((int)(cand[i][j])));
